"""Experiment sweep for KD text-to-code retrieval paper.

Runs training experiments from a phase 1 checkpoint and saves results
to the artifacts/ subdirectory alongside existing results.

Setup:
  This script must be run from inside the mbpp_kd_suite project directory
  (where pyproject.toml lives), so that the mbpp_kd_suite package is importable.

  cd <repo>/mbpp_kd_suite
  uv run python ../submission/experiments/run_sweep.py [options]

Model pairs:
  Pair 1 (default): TinyBERT-4L -> MiniLM-L6-v2
    Checkpoint: artifacts/two_phase_tinybert4l_taco_dt4/20260328_040837/phase1/checkpoint.pt

  Pair 2: MiniLM-L6-v2 -> all-mpnet-base-v2
    Needs new phase 1 first:
      uv run mbpp-kd-two-phase \\
        ...
        --phase2-epochs 1 --methods score_distill
    Then pass --checkpoint <path>/phase1/checkpoint.pt

Usage:
  # Run all sets (1-4) with existing pair 1 checkpoint:
  uv run python ../submission/experiments/run_sweep.py

  # Run specific sets:
  uv run python ../submission/experiments/run_sweep.py --sets 1,2

  # Resume after a crash (skips successful runs):
  uv run python ../submission/experiments/run_sweep.py --resume

  # Add a custom run (edit build_custom_runs() below):
  uv run python ../submission/experiments/run_sweep.py --sets custom
...
# Edit these to add/modify experiments. Each set is a list of RunConfig.
# Available methods:
#   control (supervised=True), score_distill, embed_distill,
#   hard_negative_pair_distill, bimga, bimga_uniform, bimga_query_only
...
def build_all_runs() -> dict[str, list[RunConfig]]:
    ...
    # ── Set 2: Batch Size Ablation (bs=64, seed=42, best configs) ───────
    set2 = []
    ...
    # Pass a different --checkpoint for a different student-teacher pair.
    set5 = []
    ...
    # ── Custom: Add your own runs here ──────────────────────────────────
    # Uncomment and edit to add custom experiments:
    # custom = []
    # custom.append(RunConfig(name="custom_bimga_dw200", method="bimga", distill_weight=200, align_weight=10))
    # custom.append(RunConfig(name="custom_embed_dw200", method="embed_distill", distill_weight=200, align_weight=10))
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Run KD experiment sweep. Results are saved to the artifacts/ subdirectory.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog=__doc__,
    )
    parser.add_argument("--sets", default="1,2,3,4",
                        help="Comma-separated set numbers to run (e.g., '1,2,3,4' or '5' or 'custom')")
    parser.add_argument("--checkpoint", default=CHECKPOINT,
                        help="Phase 1 checkpoint path")
    parser.add_argument("--resume", action="store_true",
                        help="Resume into existing artifacts dir (skips completed runs, retries failed)")
    args = parser.parse_args()

    # Parse requested sets
    requested_sets: set[int] = set()
    for s in args.sets.split(","):
        s = s.strip()
        if s.isdigit():
            requested_sets.add(int(s))
        elif s == "custom":
            requested_sets.add(6)  # custom maps to set6
    ...
    # Output to artifacts/ subdirectory
    ...
    total_runs = sum(len(all_sets[s]) for s in set_names if int(s[3]) in requested_sets)
    completed = 0

    for set_name in set_names:
        set_num = int(set_name[3])
